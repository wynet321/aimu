...
namespace aimu
{
    public partial class OMDeleteOperateDialog : Form
    {
        public OMDeleteOperateDialog()
        {
            InitializeComponent();
        }
        ...
        public OMDeleteOperateDialog(string customerName, string customerId)
        {
            InitializeComponent();
            ...
            textBox1.Text = "客户名称：" + customerName + "，如果管理员需要删除订单，请先选择订单，然后点击删除按钮";
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            try
            {
                if (dataGridView1.SelectedCells.Count > 0)
                {
                    ...
                    DialogResult dialogResult = MessageBox.Show("请确定是否要把客户:" + thecustomerName + " 的订单：" + orderID + " 删除？", "退出", MessageBoxButtons.YesNo);
                    if (dialogResult == DialogResult.Yes)
                    {
                        TruncateTable.deleteCustomerOrderByOrderID(orderID);
                        MessageBox.Show("客户:" + thecustomerName + " 的订单：" + orderID + " 已删除！");
